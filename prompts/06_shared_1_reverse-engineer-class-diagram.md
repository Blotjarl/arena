# Prompt 06_shared_1 — Reverse-Engineer the Code into a UML Class Diagram

### CRITICAL DIRECTIVE ###
**CRITICAL: Load `prompts/00_master_context.md` before proceeding. Confirm `main` includes PR #10
(`docs/api/` exists and `npm run typecheck --workspaces` passes) before starting — this prompt reads the
whole current codebase and needs Steps 2–5 to be genuinely finished first.** This prompt implements
`docs/ProjectProcess.txt` Step 6: "Reverse engineer the code in the current state into a UML class
diagram." **MANDATORY**: this must be *generated*, not hand-authored — Step 1's diagram
(`docs/01_class_diagram.html`) was a human/AI judgment call about what the system *should* look like;
this one has to mechanically reflect what the code *actually* is, so that re-running it later (Step 12)
on changed code produces a comparably faithful result. Do not describe classes from memory or from
`docs/01_class_list.md` — every fact in the output must come from parsing the real source.

---

### MANDATORY: Sandwich Requirement
- **Start**: `docs/01_class_diagram.html` (Step 1) exists and is untouched by this prompt — it is the
  comparison baseline, not something to edit here.
- **End**: `docs/06_class_diagram_reverse-engineered.html` exists, was produced by running
  `scripts/generate-class-diagram.js` (not written by hand), renders real Mermaid diagrams (not raw
  syntax text) for all four packages, and the generator script itself is committed so Step 12 can reuse it
  unchanged.

---

### 1. Why a script, not a prompt asking you to "look at the code and draw a diagram"

Marshall's earlier requirement (this exact question was raised before Step 1's diagram existed): a
reverse-engineered diagram is only useful for comparison against Step 1 if regenerating it is
**deterministic** — same code in, byte-for-byte-equivalent structure out, every time, regardless of which
session or which day runs it. An LLM re-describing the codebase from scratch each time would drift in
small ways (different wording, different ordering, different judgment calls) that have nothing to do with
whether the *code* actually changed. A script reading the TypeScript AST doesn't have that problem.

**MANDATORY**: `packages/shared` already has `typedoc` installed and configured (`typedoc.json`,
`tsconfig.typedoc.json`, from Step 5) and it correctly resolves every type across all four packages
(verified in that step). Reuse it — the generator below runs `npx typedoc --json` to get a full structured
model of the codebase, then walks that JSON (not the raw `.ts` files) to build the diagram. This means the
same code that already renders correctly at `docs/api/` is now feeding the diagram, guaranteeing they
never disagree.

---

### 2. Create `scripts/generate-class-diagram.js` — exact content, do not modify

This has already been written and validated against this exact repository (real Mermaid rendering
confirmed in a browser, zero console errors, correct relationship extraction spot-checked across
`shared`/`server`/`client`/`api`, self-contained with zero external network requests). Create the file
with this content verbatim:
...
```
node scripts/generate-class-diagram.js
```

**MANDATORY checks before committing:**
- Exit code 0, and the console line reports a file size in the low single-digit MB (it embeds the same
  ~3.5MB Mermaid runtime `docs/01_class_diagram.html` already carries — that's expected, not a bug).
- Open `docs/06_class_diagram_reverse-engineered.html` in a browser (or serve it locally and check via
  whatever browser tooling you have) and confirm the diagrams actually render as boxes and arrows, not raw
  `classDiagram ...` text — if you see literal mermaid syntax on the page instead of a rendered diagram,
  something about the escaping broke and you must stop and investigate rather than committing broken output.
- Confirm all six sections are present: shared MVC framework, shared domain, shared exceptions, server,
  client, api.
- Confirm `git status` shows **no changes under `packages/`** — this script only reads source, it must
  never modify it. If anything under `packages/` shows as modified, stop; do not commit; investigate why
  (most likely cause: a stray `.typedoc-model.json` wasn't cleaned up, or `npx typedoc` regenerated
  `docs/api/` with different line endings as a side effect — if so, `git checkout -- docs/api/` to discard
  that and re-verify, since `docs/api/` regeneration is not this prompt's job).

### 4. Add a convenience script to root `package.json`
```json
"scripts": {
  "diagram:reverse-engineer": "node scripts/generate-class-diagram.js"
}
```
(add alongside the existing `build`/`typecheck`/`test`/`docs` scripts)

---

### 5. Verification and Git
Per master context §9.5/§9.4: `npm run typecheck --workspaces` still passes (this prompt shouldn't be able
to break it, since it never touches `packages/`, but confirm anyway). Branch `shared` from `main` (`git
branch -D shared 2>/dev/null; git checkout -b shared main`), commit `Step 6: deterministic class diagram
generator + reverse-engineered diagram`, push, open a PR into `main`.

---

### CRITICAL CLOSING REQUIREMENT ###
**CRITICAL: `scripts/generate-class-diagram.js` is written once, here, and reused unmodified for Step 12's
final reverse-engineering pass — do not fork or duplicate it later. If Step 12 needs different behavior,
that means this script needs to change for both call sites, not that a second script gets written.**
